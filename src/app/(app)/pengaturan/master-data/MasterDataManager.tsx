...
/**
 * Satu baris item + editornya (AI-29, B-29).
 *
 * Pop-up modal (`<dialog>` native) -- pola OpRecordEditor.tsx, bukan lagi
 * <details> inline: dulu dipilih <details> supaya pembungkus tabel yang
 * overflow-x-auto tidak memotong panel (overflow-x non-visible membuat
 * overflow-y ikut memotong popover `absolute`). `<dialog>` tidak kena
 * masalah itu -- dirender di top layer browser, di luar alur overflow tabel
 * sama sekali. m-auto untuk penengahan (preflight Tailwind menghapus margin
 * bawaan dialog).
 *
...
function ItemRow({
  ...
  useEffect(() => {
    if (editState.ok) dialogRef.current?.close();
  }, [editState.ok]);
          ...
          {item.isGlobal ? (
            ...
            <>
              <button
                ...
                className="inline-flex items-center gap-1 whitespace-nowrap rounded-md border border-slate-200 bg-white px-2 py-1 text-xs font-medium text-slate-600 hover:bg-slate-50"
              >
              ...
              <dialog
                ...
                className="m-auto w-[min(92vw,28rem)] rounded-xl border border-slate-200 bg-white p-0 text-left shadow-xl backdrop:bg-slate-900/50"
              >
              <form
              ...
              >
                <div className="mb-1 flex items-center justify-between border-b border-slate-100 pb-3">
                  <h2 className="text-sm font-semibold text-slate-800">Ubah item &mdash; {item.name}</h2>
                  <button
                    type="button"
