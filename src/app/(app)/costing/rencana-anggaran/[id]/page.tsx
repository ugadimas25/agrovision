  ...
  return (
    <div>
      ...
      <div className="mb-5 grid grid-cols-1 gap-3 sm:grid-cols-3">
        <Kartu label="Subtotal komponen" nilai={plan.subtotalIdr} />
        <Kartu
          label={`Kontingensi ${formatNumber(plan.contingencyPct)}%`}
          nilai={plan.subtotalIdr === null ? null : Math.round(plan.subtotalIdr * plan.contingencyPct / 100)}
        />
          ...
          <div className="overflow-hidden rounded-xl border border-slate-200 bg-white">
            <ResponsiveTable>
              <table className="w-full text-sm">
                ...
                <tbody>
                  {items.map((it) => (
                    <tr key={it.id} className="border-b border-slate-50 align-top last:border-0">
