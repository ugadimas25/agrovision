# -*- coding: utf-8 -*-
"""
Sisipkan kelompok skenario login L-01..L-07 ke sheet QA manual.
...
sheet "Skenario QA" (sheet "Petunjuk" dan "Log Bug" TIDAK disentuh).

Kenapa lewat skrip, bukan diedit tangan: penyisipannya bisa diulang, bisa
diperiksa orang lain, dan idempoten — kalau L-01 sudah ada, skrip berhenti
...
def main():
    ...
    if sudah:
        print("Kelompok L sudah ada di sheet (%s) — tidak ada yang disisipkan." % ", ".join(sudah))
        return 0
    if hanya_cek:
        print("Kelompok L BELUM ada. %d baris skenario siap disisipkan." % len(BARIS))
        return 0
    ...
    print("Disisipkan ke %s → sheet '%s': 1 kepala kelompok + %d skenario "
          "(baris %d-%d)." % (os.path.basename(XLSX), SHEET, len(BARIS),
                              baris_baru[0], baris_baru[-1]))

    # Verifikasi: baca ULANG dari disk, bukan dari objek yang barusan ditulis.
    ulang = openpyxl.load_workbook(XLSX)[SHEET]
    cetak_baris(ulang, baris_baru)
    return 0
