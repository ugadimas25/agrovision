...
| ~~**AI-48**~~ | ~~**Batas 8 kolom di mobile + baris detail yang bisa dibuka**~~ **SELESAI 24 Agu 2026.** Kolom sekunder ditandai TERPUSAT di `buildReportScreen()` (`SCREEN_DETAIL_COLUMNS`, dicocokkan lewat label) — 15 builder tidak disentuh. Mobile: kartu menampilkan ≤8 baris, sisanya di balik `<details>` native pada kartu yang sama. Desktop/PDF/Excel tidak dipangkas — batas 8 itu batas layar sempit, bukan batas dokumen. Dijaga `at:verify`. **Koreksi angka §18c:** maksimum sebenarnya **13 kolom**, bukan 16, dan **13 dari 15** melampaui 8 (karbon & pengeluaran sudah pas) — estimasi 4–5 hari disusun dari angka yang lebih buruk. | catatan 7, G-04 · K-07 | P1 | L (4–5 hari) | Frontend |
...
**AI-47 dan AI-48 sudah selesai (24 Agu 2026).** AI-38 (uji ekspor F-01…F-05) kini bebas jalan — dan itu urutan yang dimaksud: pemilahan kolom sudah tetap, jadi yang diuji tidak akan dibuang. Membalik urutan berarti mengerjakan tata letak dua kali dan menguji jalur yang akan dibuang. AI-37 dan AI-49 bisa jalan paralel.
